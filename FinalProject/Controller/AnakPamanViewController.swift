...
class AnakPamanViewController: UIViewController {

    @IBOutlet weak var anakPamanKandungTextField: UITextField!
    @IBOutlet weak var anakPamanSeayahTextField: UITextField!
    @IBOutlet weak var skipAnakPamanSeayahLabel: UILabel!
    ...
    var jmlAnakPamanKandung: Int {
        if anakPamanKandungTextField.text == "" {
            return 0
        }
        return Int(anakPamanKandungTextField.text!)!
    }
    
    var jmlAnakPamanSeayah: Int {
        if anakPamanSeayahTextField.text == "" {
            return 0
        }
        return Int(anakPamanSeayahTextField.text!)!
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        skipAnakPamanSeayahLabel.isHidden = true
        anakPamanKandungTextField.addTarget(self, action: #selector(textFieldDidEndEditing), for: .editingChanged)
    }

    @IBAction func buttonNextPressed(_ sender: Any) {
        setAbstractionLocalVariables()
        performSegue(withIdentifier: "toEndFromAnakPaman", sender: nil)
    }
    
    func setAbstractionLocalVariables(){
        jumlahAnakPamanKandung = jmlAnakPamanKandung
        jumlahAnakPamanSeayah = jmlAnakPamanSeayah
        
        if jumlahAnakPamanKandung > 0 {
            isAnakPamanKandung = true
        } else if jumlahAnakPamanKandung <= 0 {
            isAnakPamanKandung = false
        }
        
        if jumlahAnakPamanSeayah > 0 {
            isAnakPamanSeayah = true
        } else if jumlahAnakPamanSeayah <= 0 {
            isAnakPamanSeayah = false
        }
        
        if isAnakPamanSeayah || isAnakPamanKandung {
            isAnakPaman = true
        } else {
            isAnakPaman = false
        }
    ...
    func checkAshabah(){
        if isAnakPamanKandung == true {
            isAnakPamanKandungAshabah = true
            isAshabah = true
            
        } else if isAnakPamanKandung == false {
            isAnakPamanKandungAshabah = false
            isAshabah = false
            
            if isAnakPamanSeayah == true {
                isAnakPamanSeayahAshabah = true
                isAshabah = true
            } else if isAnakPamanSeayah == false {
                isAnakPamanSeayahAshabah = false
                isAshabah = false
            }
        }
    }
    ...
    @objc func textFieldDidEndEditing(_ textField: UITextField) {
        let _jumlahAnakPamanKandung = Int(textField.text!) ?? 0
        if _jumlahAnakPamanKandung >= 1 {
            anakPamanSeayahTextField.isEnabled = false
            anakPamanSeayahTextField.text = ""
            skipAnakPamanSeayahLabel.isHidden = false
        }
        else {
            anakPamanSeayahTextField.isEnabled = true
            skipAnakPamanSeayahLabel.isHidden = true
        }
